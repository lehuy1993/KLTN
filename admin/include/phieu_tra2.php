...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        for ($i=0; $i<sizeof($checkBox); $i++)
        {
            ...
            $query="SELECT * FROM ct_thue Where id = " . $checkBox[$i] . " ";
             $db = mysql_query($query)or die(mysql_error());


             while ($dong = mysql_fetch_array($db)) {

            # code...
            $tien_thue +=  $dong['tong_tien'];
            $tien_cuoc += $dong['tien_cuoc'];
            $tong_tien = $tien_cuoc + $tien_thue;
...
if (isset($_POST['add']) ) {
	...
	if($dbAction->insert(Table::tbphieu_tra,$data)==true)
	{
         ...
         for ($k=0; $k<count($checkBox); $k++)
        {
            ...
            $query1="SELECT * FROM ct_thue Where id = '$checkBox[$k]' ";

            $db1 = mysql_query($query1);

             while ($dong1 = mysql_fetch_array($db1) ) {
            # code...
             $sql = "UPDATE ct_truyen SET so_luong = so_luong + '$dong1[so_luong]'  Where id_truyen2 = '$dong1[id_truyen]'  ";
              mysql_query($sql);
            }
        }
        ...
        for ($j=0; $j<sizeof($checkBox); $j++){
            $delete = "DELETE FROM ct_thue Where id =" . $checkBox[$j] . "";
            mysql_query($delete);
        }
        ...
        if ($data1 == 0) {
            ...
            $where1 = array('id'=>$id);
            $dbnew->delete(Table::tbthue,$where1);
        }
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active" id="jsncc">
                                    <table class="table table-striped table-bordered ">
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td style="text-align: center;">
                                                        ...
                                                        <?php echo number_format($tien_cuoc);?>
                                                    </td>
                                                    <td style="text-align: center;">

                                                        <?php echo number_format($tien_thue);?>
                                                    </td>
                                                    <td style="text-align: center;">
                                                        <?php echo number_format($tong_tien);?>
                                                    </td>
                                                    <td style="text-align: center;">
                                                ...
                                                <?php 
                                                 for ($i=0; $i<sizeof($checkBox); $i++)
                                                    { echo "
                                                 <input type='hidden' name='checkBox[]'   value='$checkBox[$i]'>";
                                    	...
                                    	<input type="hidden" name="tien_coc" id="input" class="form-control" value="<?php echo $tien_cuoc ?>">
                                    	<input type="hidden" name="tong_tien" id="input" class="form-control" value="<?php echo $tong_tien ?>">
										...
										<input type="hidden" name="tien_thue" id="input" class="form-control" value="<?php echo $tien_thue ?>">
